...
class ZmqBridge(BasicSdk2Bridge):
    ...
    def publish_low_state(self):
        ...
        # MuJoCo's freejoint angular velocity (qvel[3:6]) is ALREADY in the body frame
        # (MuJoCo convention) — exactly what the policy expects (IMU = body-frame gyro).
        # Pass it through directly. NOTE: a previous world->body rotation here was a bug:
        # while turning, the accumulating yaw angle rotated the roll/pitch angular-velocity
        # components and corrupted the balance feedback -> 100% fall on yaw/mixed commands
        # (translation was unaffected because yaw stays ~0). Removing it fixes turning with
        # no retraining.
